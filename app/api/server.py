...
def list_documents():
    ...
    return [{"id": str(d.id), "filename": d.filename, "status": d.status, "page_count": d.page_count} for d in docs]
...
def query(session_id: str = Form(None), query: str = Form(...), top_k: int = Form(3)):
    ...
    if not hits:
        ...
        crud.create_message(session.id, "assistant", answer, meta={})
    ...
    assistant_msg = crud.create_message(session.id, "assistant", answer, meta={"sources": hits})
    crud.create_telemetry(session.id, assistant_msg.id, query, [h.get("chunk_id") for h in hits],
                          retrieval_time_ms, llm_resp["llm_time_ms"], top_k)
